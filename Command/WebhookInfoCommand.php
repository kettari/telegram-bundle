...
class WebhookInfoCommand extends AbstractCommand
{
  ...
  protected function executeCommand()
  {
    ...
    $this->io->writeln(
      'Url: '.(empty($hookInfo->url) ? '(empty)' : $hookInfo->url)
    );
    $this->io->writeln(
    ...
    $this->io->writeln(
      'Pending update count: '.$hookInfo->pending_update_count
    );
    $this->io->writeln(
      'Last error date: '.($hookInfo->last_error_date > 0 ? date(
        'r',
        $hookInfo->last_error_date
      ) : '(none)')
    );
    $this->io->writeln(
      'Last error message: '.
    ...
    $this->io->section('Vanity information');
    /** @var \Doctrine\Bundle\DoctrineBundle\Registry $d */
    $d = $this->getContainer()
      ->get('doctrine');
    /** @var \Kettari\TelegramBundle\Entity\BotSetting $setting */
    if (is_null(
      $setting = $d->getRepository('KettariTelegramBundle:BotSetting')
        ->findOneByName('bot_user_id')
    )) {
      $this->io->writeln('No self user ID in the database.');
    } else {
      $this->io->writeln('Bot user ID: '.$setting->getValue());
    }
