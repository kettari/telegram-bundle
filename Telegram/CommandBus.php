...
class CommandBus {
  ...
  public function executeCommand($command_name, Update $update) {
    ...
    foreach ($this->commands_classes as $command_class => $placeholder) {
      ...
      if ($command_class::getName() == $command_name) {
        ...
        if (!$this->isAuthorized($update->message->from, $command_class)) {
          $l->notice('User is not authorized to execute /{command_name} command with the class "{class_name}"',
        ...
        // User is authorized
        $l->info('Executing /{command_name} command with the class "{class_name}"',
          ['command_name' => $command_name, 'class_name' => $command_class]);
    ...
    $l->notice('No class registered to handle /{command_name} command',
  ...
  protected function isAuthorized(TelegramUser $tu, $command_class) {
    ...
    die;*/
    $applicable_permissions = array_intersect($required_permissions,
      $user_permissions);

    // Write to the log permissions check result
    $l->info('Authorization check', [
